...
exports.calendarService = {
    ...
    async createCalendar(data, imageFile) {
        let imageUrl = '';
        if (imageFile) {
            const uploadResult = await cloud_upload_service_1.cloudUploadService.uploadImage(imageFile, 'calendars');
        ...
        if (imageFile) {
            ...
            if (existingCalendar.imageUrl) {
                try {
                    ...
                    if (publicId) {
                        await cloud_upload_service_1.cloudUploadService.deleteFile(publicId);
                    }
                }
                catch (error) {
