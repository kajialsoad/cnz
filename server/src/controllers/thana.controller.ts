...
import { ThanaStatus } from '@prisma/client';
...
class ThanaController {
    ...
    async getThanas(req: Request, res: Response) {
        try {
            ...
            const validStatus = status as ThanaStatus | 'ALL' | undefined;

            const thanas = await thanaService.getThanasByCityCorporation(
                cityCorporationCode,
                validStatus || 'ALL'
            );
     ...
     */
    async getThanaById(req: Request, res: Response) {
        try {
            const { id } = req.params;
     ...
     * Create new thana
     */
    async createThana(req: Request, res: Response) {
        try {
     ...
     * Update thana
     */
    async updateThana(req: Request, res: Response) {
        try {
            const { id } = req.params;
            const thanaId = parseInt(id);
     ...
     */
    async getThanaStatistics(req: Request, res: Response) {
        try {
            const { id } = req.params;
            const thanaId = parseInt(id);
...
export default new ThanaController();
