import { PrismaClient, ThanaStatus } from '@prisma/client';
...
class ThanaService {
     ...
     * Create new thana
     */
    async createThana(data: CreateThanaDto) {
        ...
        if (existing) {
            throw new Error(
                `Thana with name "${data.name}" already exists in ${cityCorporation.name}`
            );
        }
     ...
     * Update thana
     */
    async updateThana(id: number, data: UpdateThanaDto) {
        ...
        if (data.name && data.name !== existing.name) {
            ...
            if (duplicate) {
                throw new Error(
                    `Thana with name "${data.name}" already exists in this city corporation`
                );
            }
        }
     ...
     */
    async validateThanaBelongsToCityCorporation(
        thanaId: number,
        cityCorporationCode: string
    ): Promise<boolean> {
        const thana = await prisma.thana.findUnique({
...
export default new ThanaService();
