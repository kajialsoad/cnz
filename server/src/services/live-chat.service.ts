...
private checkAdminAccess(admin: { role: string; wardId: number | null; zoneId: number | null; cityCorporationCode: string | null }, user: { wardId: number | null; zoneId: number | null; cityCorporationCode: string | null }): boolean { if (admin.role === 'MASTER_ADMIN') { return admin.cityCorporationCode === user.cityCorporationCode; } if (admin.role === 'SUPER_ADMIN') { return admin.zoneId === user.zoneId; } if (admin.role === 'ADMIN') { return admin.wardId === user.wardId; } return false; }
}
