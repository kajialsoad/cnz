...
export class ReviewService {
     ...
     */
    async submitReview(input: ReviewSubmissionInput): Promise<Review> {
        try {
            ...
            await notificationService.notifyAdmins(
                ...
                `A new ${rating}-star review has been submitted for a resolved complaint.`,
                'SUCCESS',
                complaintId
            );
     ...
     */
    async getReviewsByComplaint(complaintId: number): Promise<any[]> {
        try {
     ...
     */
    async getReviewAnalytics(filters: ReviewAnalyticsFilters = {}): Promise<ReviewAnalytics> {
        try {
     ...
     */
    async getAverageRating(filters: ReviewAnalyticsFilters = {}): Promise<number> {
        try {
            const { cityCorporationCode, zoneId, wardId, startDate, endDate } = filters;
     ...
     */
    async getRatingDistribution(filters: ReviewAnalyticsFilters = {}): Promise<RatingDistribution> {
        try {
            const { cityCorporationCode, zoneId, wardId, startDate, endDate } = filters;
     ...
     */
    async canSubmitReview(complaintId: number, userId: number): Promise<boolean> {
        try {
            ...
            if (complaint.status !== Complaint_status.RESOLVED || complaint.userId !== userId) {
                return false;
            }
