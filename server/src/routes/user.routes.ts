...
// Update user profile
router.put('/profile', authGuard, async (req: AuthRequest, res) => {
  try {
    const userId = req.user!.sub.toString();
    const { error, value } = validateInput(updateProfileSchema, req.body);
    ...
    const profile = await authService.updateProfile(userId, value);
...
router.post('/change-password', authGuard, async (req: AuthRequest, res) => {
  try {
    const userId = req.user!.sub.toString();
    const { error, value } = validateInput(changePasswordSchema, req.body);
...
router.put('/admin/users/:id/status', authGuard, rbacGuard('ADMIN', 'SUPER_ADMIN'), async (req, res) => {
  try {
    ...
    const user = await (req as any).prisma.user.update({
      where: { id: parseInt(id) },
      data: { status },
      select: {
        id: true,
